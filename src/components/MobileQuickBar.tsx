...
export const MobileQuickBar = ({ phoneTel, address }: MobileQuickBarProps) => {
  ...
  return (
    <nav aria-label="Mobile quick actions" className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#061a17]/95 backdrop-blur-xl border-t border-slate-800/90 py-2.5 px-3 flex items-center justify-around shadow-[0_-10px_25px_rgba(0,0,0,0.5)]">
      <a
        href={phoneTel}
        className="flex flex-col items-center gap-1 text-[11px] font-bold text-emerald-400 active:scale-95 transition-transform"
        aria-label="Call Office — clinic front desk"
      >
      ...
      <a
        ...
        aria-label="Doctors — view our physicians"
      >
      ...
      <a
        href="#forms"
        className="flex flex-col items-center gap-1 text-[11px] font-bold text-teal-400 active:scale-95 transition-transform"
        aria-label="Forms PDF — download patient registration forms"
      >
        <div className="w-8 h-8 rounded-full bg-teal-500/20 flex items-center justify-center">
      ...
      <a
        ...
        aria-label="Directions — open clinic location in Google Maps (new tab)"
      >
